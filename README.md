...
支持教务系统导出的 `.xls` / `.xlsx` / `.csv`，**不需要 API Key、零识别误差**。
也可以把 Excel 里的课表整块复制，粘贴到页面上的文本框里解析。

已适配的单元格格式（多门课用换行分隔，字段用 `/` 分隔）：

```
课程名/(1-2节)1-17周(单)/ 一教204/吴菁菁/班级/人数
```

支持 `1-17周(单)`、`2-18周(双)`、`1-8,10-16周`、`12周` 等写法。
表格注释里若有「本学期2026-09-07正式上课…共20周」，学期起始日与总周数会自动写入设置。

解析后会列出所有课程，**点任意一条可展开修改**（星期、课程名、地点、教师、周次表达式），点左侧方框取消导入。

### 2. 图片识别

只有课表截图时用。需要配置 API Key（见下）。

## 配置识别 API（可选）

不配置也能用——用上面的「表格文件」导入，或手动添加课程。配置后图片识别可用：

- 打开 App → 设置 → 课表识别 → 选「智谱」→ 填入 API Key → **测试连接**
- 智谱 GLM-4V-Flash 永久免费：https://open.bigmodel.cn
- 阿里百炼 Qwen2.5-VL：https://dashscope.aliyun.com

## 本地验证脚本

需要先 `npm i xlsx`（表格解析依赖，App 内已自带 `www/vendor/xlsx.full.min.js`）。

```bash
# 用真实课表跑一遍：解析 → 展开周次 → 排提醒，打印前 8 条提醒
node tools/verify-schedule.js "D:/zz/25现代物流管理(01)班课表.xls"

# 测视觉识别链路（需 API Key）
python tools/verify-vision.py <API_KEY> [图片路径]
python tools/make-sample-timetable.py      # 生成一张仿真实课表图用于测试
```
